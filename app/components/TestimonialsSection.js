...
const TestimonialsSection = () => {
  ...
  const [currentTestimonial, setCurrentTestimonial] = useState(0);
  const cloudinaryBaseUrl = 'https://res.cloudinary.com/dq6dotbrs/image/upload';
  ...
  const fetchTestimonials = async () => {
    try {
      ...
      if (docSnap.exists()) {
        const fetchedTestimonials = docSnap.data().testimonials || [];

        // Map through testimonials to ensure Cloudinary URLs are used for images
        const testimonialsWithImages = fetchedTestimonials.map((testimonial) => ({
          ...testimonial,
          image: testimonial.image.startsWith('http')
            ? testimonial.image // Already a URL
            : `${cloudinaryBaseUrl}/${testimonial.image}`, // Construct URL for Cloudinary
        }));
  ...
  // Automatically transition to the next testimonial
  useEffect(() => {
